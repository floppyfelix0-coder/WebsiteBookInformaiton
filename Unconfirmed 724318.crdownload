...
function publishFromWriter() {
    ...
    document.querySelectorAll('.writer-chapter-item').forEach((item, index) => {
        ...
        const pages = Math.ceil(words / 250); // Estimate 250 words per page
    ...
    const bookData = {
        ...
        description: `A book with ${chapters.length} chapters and ${chapters.reduce((sum, ch) => sum + ch.wordCount, 0)} words.`,
        coverUrl: '',
        chapters: chapters
    };
